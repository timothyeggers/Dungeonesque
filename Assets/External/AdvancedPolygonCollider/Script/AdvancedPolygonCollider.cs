...
namespace DigitalRuby.AdvancedPolygonCollider
{
    public struct PolygonParameters
    {
        ...
        {
            var h = Texture.GetHashCode();
            if (h == 0) h = 1;
            return h * (int)(Rect.GetHashCode() * XMultiplier * YMultiplier * AlphaTolerance *
                             Mathf.Max(DistanceThreshold, 1) * (Decompose ? 2 : 1));
        }
    ...
    public class AdvancedPolygonCollider : MonoBehaviour
    {
        ...
        [Tooltip("Pixels with alpha greater than this count as solid.")] [Range(0, 254)]
        public byte AlphaTolerance = 20;

        [Tooltip("Points further away than this number of pixels will be consolidated.")] [Range(0, 64)]
        public int DistanceThreshold = 8;

        [Tooltip("Scale of the polygon.")] [Range(0.5f, 2.0f)]
        ...
        [Tooltip(
            "True to use the cache, false otherwise. The cache is populated in editor and play mode and uses the most recent geometry " +
        ...
        [SerializeField] [HideInInspector] private byte lastAlphaTolerance;

        [SerializeField] [HideInInspector] private float lastScale;

        [SerializeField] [HideInInspector] private int lastDistanceThreshold;

        [SerializeField] [HideInInspector] private bool lastDecompose;

        [SerializeField] [HideInInspector] private Sprite lastSprite;

        [SerializeField] [HideInInspector] private Rect lastRect;

        [SerializeField] [HideInInspector] private Vector2 lastOffset = new(-99999.0f, -99999.0f);

        [SerializeField] [HideInInspector] private float lastPixelsPerUnit;

        [SerializeField] [HideInInspector] private bool lastFlipX;

        [SerializeField] [HideInInspector] private bool lastFlipY;

        [Tooltip("All the cached objects from the editor. Do not modify this data.")] [SerializeField]
        private List<CacheEntry> editorCache = new();
        ...
        public int VerticesCount => polygonCollider == null ? 0 : polygonCollider.GetTotalPointCount();
        ...
        private void Awake()
        {
            if (Application.isPlaying)
                ...
                foreach (var v in editorCache)
                {
                    ...
                    cache[v.Key] = list;
                    foreach (var w in v.Value.List) list.Add(w.Array);
                }
        }
        ...
        public void UpdatePolygonCollider(ref PolygonParameters p)
        {
            ...
            if (Application.isPlaying && p.UseCache)
            {
                ...
                if (cache.TryGetValue(key, out cached))
                {
                    ...
                    for (var i = 0; i < cached.Count; i++) polygonCollider.SetPath(i, cached[i]);
        ...
        public void PopulateCollider(PolygonCollider2D collider, ref PolygonParameters p)
        {
            try
            {
                if (p.Texture.format != TextureFormat.ARGB32 && p.Texture.format != TextureFormat.BGRA32 &&
                    p.Texture.format != TextureFormat.RGBA32 &&
                    p.Texture.format != TextureFormat.RGB24 && p.Texture.format != TextureFormat.Alpha8 &&
                    p.Texture.format != TextureFormat.RGBAFloat &&
                    p.Texture.format != TextureFormat.RGBAHalf && p.Texture.format != TextureFormat.RGB565)
                    Debug.LogWarning(
                        "Advanced Polygon Collider works best with a non-compressed texture in ARGB32, BGRA32, RGB24, RGBA4444, RGB565, RGBAFloat or RGBAHalf format");
                var width = (int)p.Rect.width;
                var height = (int)p.Rect.height;
                var x = (int)p.Rect.x;
                var y = (int)p.Rect.y;
                var pixels = p.Texture.GetPixels(x, y, width, height, 0);
                var verts = geometryDetector.DetectVertices(pixels, width, p.AlphaTolerance);
                var pathIndex = 0;
                var list = new List<Vector2[]>();

                for (var i = 0; i < verts.Count; i++) ProcessVertices(collider, verts[i], list, ref p, ref pathIndex);
        ...
        private List<Vector2[]> ProcessVertices(PolygonCollider2D collider, Vertices v, List<Vector2[]> list,
            ref PolygonParameters p, ref int pathIndex)
        {
            var offset = p.Offset;
            var flipXMultiplier = spriteRenderer.flipX ? -1.0f : 1.0f;
            var flipYMultiplier = spriteRenderer.flipY ? -1.0f : 1.0f;

            if (p.DistanceThreshold > 1) v = SimplifyTools.DouglasPeuckerSimplify(v, p.DistanceThreshold);

            if (p.Decompose)
            {
                var points = BayazitDecomposer.ConvexPartition(v);
                for (var j = 0; j < points.Count; j++)
                {
                    var v2 = points[j];
                    for (var i = 0; i < v2.Count; i++)
                    {
                        var xValue = 2.0f * ((v2[i].x - offset.x + 0.5f) / p.Rect.width);
                        var yValue = 2.0f * ((v2[i].y - offset.y + 0.5f) / p.Rect.height);
                        v2[i] = new Vector2(xValue * p.XMultiplier * Scale * flipXMultiplier,
                            yValue * p.YMultiplier * Scale * flipYMultiplier);
                    }
            ...
            else
            {
                ...
                {
                    var xValue = 2.0f * ((v[i].x - offset.x + 0.5f) / p.Rect.width);
                    var yValue = 2.0f * ((v[i].y - offset.y + 0.5f) / p.Rect.height);
                    v[i] = new Vector2(xValue * p.XMultiplier * Scale * flipXMultiplier,
                        yValue * p.YMultiplier * Scale * flipYMultiplier);
                }
        ...
        {
            [SerializeField] public Vector2[] Array;
        }

        [Serializable]
        public struct ListWrapper
        {
            [SerializeField] public List<ArrayWrapper> List;
        }

        [Serializable]
        public struct CacheEntry
        {
            [SerializeField] public CacheKey Key;

            [SerializeField] public ListWrapper Value;
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            var text = " Vertices: " + VerticesCount + " ";
            var view = SceneView.currentDrawingSceneView;
            var screenPos = view.camera.WorldToScreenPoint(gameObject.transform.position);
        ...
        private void AddEditorCache(ref PolygonParameters p, List<Vector2[]> list)
        {
            ...
            {
                var w = new ArrayWrapper();
                w.Array = v;
                e.Value.List.Add(w);
            }

            for (var i = 0; i < editorCache.Count; i++)
