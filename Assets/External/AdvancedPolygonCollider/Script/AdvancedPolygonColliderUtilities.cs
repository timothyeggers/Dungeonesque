...
namespace DigitalRuby.AdvancedPolygonCollider
{
    public sealed class TextureConverter
    {
        ...
        private static readonly int[,] closePixels = new int[closePixelsLength, 2]
        ...
        public List<Vertices> DetectVertices(Color[] colors, int width, int alphaTolerance)
        {
            ...
            for (var i = 0; i < solids.Length; i++)
            {
                ...
                s = (int)((n & 0x80000000) >> 31) -
                    1; // sign multiplier is -1 for positive numbers and 0 for negative numbers
            ...
            var detectedVerticesList = DetectVertices();
        ...
        public List<Vertices> DetectVertices()
        {
            ...
            do
            {
                ...
                if (detectedPolygons.Count == 0)
                {
                    ...
                    if (polygon.Count > 2) polygonEntrance = GetTopMostVertex(polygon);
                }
                            ...
                            if (!blackList.Contains(holeEntrance.Value))
                            {
                                ...
                                if (holePolygon != null && holePolygon.Count > 2)
                                {
                                    ...
                                    if (SplitPolygonEdge(polygon, holeEntrance.Value, out vertex1Index,
                                            out vertex2Index)) polygon.InsertRange(vertex2Index, holePolygon);
                                    break;
                                }
                            }
                            else
        ...
        {
            for (var i = 0; i < detectedPolygons.Count; i++) detectedPolygons[i].Reverse();
        }
        ...
        /// <summary>
        ///     Function to search for an entrance point of a hole in a polygon. It searches the polygon from top to bottom between
        ///     the polygon edges.
        /// </summary>
        ...
        private Vector2? SearchHoleEntrance(Vertices polygon, Vector2? lastHoleEntrance)
        {
            ...
            if (polygon.Count < 3) throw new ArgumentException("'polygon.MainPolygon.Count' can't be less then 3.");
        ...
        private bool SearchNextHullEntrance(List<Vertices> detectedPolygons, Vector2 start, out Vector2? entrance)
        {
            ...
            for (var i = (int)start.x + (int)start.y * width; i <= solidsLength; i++)
                if (IsSolid(i))
                {
                    if (foundTransparent)
                    {
    ...
    public static class BayazitDecomposer
    {
        ...
        private static bool CanSee(int i, int j, List<Vector2> vertices)
        {
            ...
            {
                if ((k + 1) % vertices.Count == i || k == i || (k + 1) % vertices.Count == j ||
                    k == j) continue; // ignore incident edges
                Vector2 intersectionPoint;
                if (LineIntersect2(At(i, vertices), At(j, vertices), At(k, vertices), At(k + 1, vertices),
                        out intersectionPoint)) return false;
            }
        ...
        {
            if (!IsCounterClockWise(vertices)) vertices.Reverse();
        }
        ...
        private static bool LineIntersect2(Vector2 a0, Vector2 a1, Vector2 b0, Vector2 b1,
            out Vector2 intersectionPoint)
        {
            intersectionPoint = Vector2.zero;
    ...
    public static class SimplifyTools
    {
        ...
        public static Vertices ReduceByArea(Vertices vertices, float areaTolerance)
        {
            ...
            for (var i = 0; i < vertices.Count; ++i, v2 = v3)
            {
                ...
                var old1 = Cross(ref v1, ref v2);
                var old2 = Cross(ref v2, ref v3);
                var new1 = Cross(ref v1, ref v3);
